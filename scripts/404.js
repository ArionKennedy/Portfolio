function fillHexGrid() {
    ...
    const rowCount = Math.ceil(containerHeight / hexHeight) + 3;
    ...
    for (let row = 0; row < rowCount; row++) {
        ...
        // Move the first row up by half a hexagon so only the bottom half shows
        if (row === 0) {
            hexRow.style.marginTop = `-${hexHeight / 2}px`;
        }
